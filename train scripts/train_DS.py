...
def train(args, train_dataloader, val_dataloader, num_val_samples, root_save_path):
    ...
    print('DS_unet')
    model.print_model_structure(DS_unet)

    DS_loss = loss.DSMapLoss().to(device)
    render_loss = loss.RenderingLoss(args.batchSize, device, args.numDiffRendering, args.numSpecRendering,
                                     args.renderSize)
    optimizer = optim.Adam(DS_unet.parameters(), lr=args.lr, betas=(0.5, 0.9))
    ...
    while True:
        for _, batch_sample in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            loss_DS = DS_loss(DS_out, materials)
            render_materials = torch.cat([materials[:, :3, :, :], DS_out[:, :3, :, :],
                                          materials[:, 6:9, :, :], DS_out[:, 3:6, :, :]], dim=1)
            loss_render = render_loss(render_materials, materials)
            loss_all = loss_DS + loss_render * 0.5
            loss_all.backward()
            optimizer.step()

            iter_took = time.time() - iter_start
            DS_loss_data = loss_DS.item()
            ...
            if total_iteration % args.displayFreq == 0:
                print(
                    '[{}] iter_took:{:.3f}s  lr:{:.6f}  l1:{:.3f}  render:{:.3f}'.format(
                        total_iteration, iter_took, lr, DS_loss_data, render_loss_data))
            ...
            if total_iteration % args.saveFreq == 0:
                ...
                torch.save(DS_unet.state_dict(), os.path.join(current_save_path, "DS_unet.pt"))
            ...
            if total_iteration % args.valFreq == 0:
                ...
                print('[Val {}] D_rmse:{:.5f}  S_rmse:{:.5f}'.format(total_iteration, avg_rmse_D, avg_rmse_S))
...
if __name__ == "__main__":
    ...
    root_save_path = utils.create_folder(
        os.path.join(args.outDir, 'i_hybridmaskgt_efficientB3_learnedDecoder_2inl_concatI_deformconv_conditionalgan_gammacorrection_NEWinl_[globalunet_DS]'), still_create=True)
